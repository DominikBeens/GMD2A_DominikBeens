...
public class Gun : Weapon
{
    ...
    public float maxChargedBonusDamage;
    public float chargeSpeed;
    private float currentChargeBonusDamage;
            ...
            if (currentAmmo > 0)
            {
                if (currentClipAmmo > 0)
                {
                    ...
                    else if (fireMode == FireMode.Charge)
                    {
                        ...
                        {
                            if (currentChargeBonusDamage < maxChargedBonusDamage)
                            {
                                currentChargeBonusDamage += Time.deltaTime * chargeSpeed;

                                if (maxChargedBonusDamage - currentChargeBonusDamage <= 0.1f)
                                {
                                    currentChargeBonusDamage = maxChargedBonusDamage;
                                }
                            }
                        }
                        if (Input.GetButtonDown("Fire1") && Time.time >= fireRateCooldown)
                        {
                            ...
                            {
                                fireRateCooldown = Time.time + 1f / fireRate;
                                Shoot(damage + currentChargeBonusDamage);
                                currentChargeBonusDamage = 0;
        ...
        if (fireRateFill != null)
        {
            if (!reloading)
            {
                ...
                if (fireMode == FireMode.Charge && scoped)
                {
                    ...
                    {
                        scopedDamageFillAmount = currentChargeBonusDamage / maxChargedBonusDamage;
